...
class chain : public LinearList<T>
{
    public:
        ...
        void radixSort(int radix, int digit);
...
template<class T>
void chain<T>::radixSort(int radix, int digit)
{
    // 初始化并创建箱子
    chainNode<T> **bin, **top;
    bin = new chainNode<T>* [radix];
    top = new chainNode<T>* [radix];
    for (int i = 0; i < radix; i++)
        bin[radix] = top[radix] = NULL;
    
    // 根据每一位数字进行排序
    for (int i = 0, n = 1; i < digit; i++, n *= radix)
    {
        for (chainNode<T> *p = firstNode; p != NULL; p = p->next)
        {
            int index = (p->element / n) % radix;
            if (bin[index] == NULL)
            {
                bin[index] = top[index] = p;
            }
            else
            {
                top[index]->next = p;
